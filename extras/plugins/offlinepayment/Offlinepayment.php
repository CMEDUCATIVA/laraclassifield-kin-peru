...
class Offlinepayment extends Payment
{
	...
	public static function sendPayment(Request $request, Post|User $payable, array $resData = [])
	{
		...
		$description = null;
		$description = $isPromoting ? trans('offlinepayment::messages.listing') : $description;
		$description = $isSubscripting ? trans('offlinepayment::messages.user') : $description;
		$description = !empty($description) ? $description . $payInfo : trim($payInfo);
		
		// API Parameters
		$params = parent::getLocalParameters($request, $payable, $package);
		$params['package']['description'] = $description;
	 ...
	 * @return array
	 */
	public static function register(Post|User $payable, array $params, array $resData = []): array
	{
