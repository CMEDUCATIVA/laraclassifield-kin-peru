...
class OpTools
{
	...
	public static function featuredCheckboxDisplay($id, $table, $field, $fieldValue = null): string
	{
		...
		if (isset($fieldValue) && $fieldValue == 1 && $validPaymentExists) {
			$html = '<i id="' . $lineId . '" class="admin-single-icon fa-solid fa-toggle-on" aria-hidden="true"></i>';
			if ($isNotBlankPayment) {
				return $html;
			}
		} else {
			$html = '<i id="' . $lineId . '" class="admin-single-icon fa-solid fa-toggle-off" aria-hidden="true"></i>';
		}
