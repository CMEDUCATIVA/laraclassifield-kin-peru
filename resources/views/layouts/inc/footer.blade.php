				...
				@if (!config('settings.footer.hide_links'))
								...
								@if (isset($pages) && $pages->count() > 0)
									@foreach($pages as $page)
										<li>
											...
											@else
												<a href="{{ \App\Services\UrlGen::page($page) }}" {!! $linkTarget !!}> {{ $page->name }} </a>
											@endif
										</li>
									@endforeach
								@endif
					...
					<div class="col">
						<div class="footer-col">
							...
							<ul class="list-unstyled footer-nav">
								<li><a href="{{ \App\Services\UrlGen::contact() }}"> {{ t('Contact') }} </a></li>
								<li><a href="{{ \App\Services\UrlGen::sitemap() }}"> {{ t('sitemap') }} </a></li>
								@if (isset($countries) && $countries->count() > 1)
									<li><a href="{{ \App\Services\UrlGen::countries() }}"> {{ t('countries') }} </a></li>
								@endif
							</ul>
						</div>
					</div>
							...
							<ul class="list-unstyled footer-nav">
								@if (!auth()->user())
									<li>
										...
										@else
											<a href="{{ \App\Services\UrlGen::login() }}"> {{ t('log_in') }} </a>
										@endif
									</li>
									<li><a href="{{ \App\Services\UrlGen::register() }}"> {{ t('register') }} </a></li>
								@else
