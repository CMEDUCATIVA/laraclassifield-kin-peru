@php
	// Clear Filter Button
	$clearFilterBtn = \App\Services\UrlGen::getCityFilterClearLink($cat ?? null, $city ?? null);
			...
			@foreach ($cities as $iCity)
				<li>
					@if (
						...
						<strong>
							<a href="{!! \App\Services\UrlGen::city($iCity, null, $cat ?? null) !!}" title="{{ data_get($iCity, 'name') }}">
								{{ data_get($iCity, 'name') }}
